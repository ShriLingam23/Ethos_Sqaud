    ...
    CourseStudent.find()
        .then((courseStudent) => {
            if(!courseStudent.length){
                ...
                res.status(200).json(cousre).send({"message":"Successfully retrived","data":courseStudent});
            }
        })
});
    ...
    CourseStudent.findById(req.params.id)
        .then((courseStudent) => {
            if(courseStudent.length){
                ...
                res.status(200).json(courseStudent).send({"message":"Successfully retrived","data":courseStudent});
            }
    })
});
...
router.put('/update/:id', (req,res) => {
    CourseStudent.updateOne({'_id':req.params.id},req.body)
        .then((courseStudent) => res.status(200).send({"message":"Successfully updated","data":courseStudent}))
        .catch((err) => res.status(400).send({"message":err}))
});
...
router.delete('/delete/:id', (req,res) => {
    CourseStudent.deleteOne({'id':req.params.regId})
        .then((courseStudent) => {
            ...
            res.status(200).send({"message":"Successfully deleted","data":courseStudent});
        })
        .catch((err) => res.status(400).send({"message":err}))
})
